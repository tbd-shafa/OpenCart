...
class Product extends \Opencart\System\Engine\Model
{
	...
	WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['category_permissions'])) {
			$category_ids = implode(',', array_map('intval', $data['category_permissions']));
			$sql .= " AND (ptc.`category_id` IN (" . $category_ids . ") OR ptc.`category_id` IS NULL)";
		} else {
			// Include products without a category
			$sql .= " AND (ptc.`category_id` IS NULL OR ptc.`category_id` IS NOT NULL)";
		}
		$sql .= " GROUP BY p.`product_id`";
